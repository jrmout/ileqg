...
{
  ros::init(argc, argv, "lwr_mpc_linear_obst_node");
  ros::NodeHandle nh("lwr_mpc_linear_obst_node");
  ...
  ileqg::IterativeLQSolverParams sparams(1e-10, 1, 1e-5, 1);
  ...
  W.setZero();
  W.topLeftCorner(x_dim/2,x_dim/2) << 1e-1*ileqg::Matrix::Identity(x_dim/2, x_dim/2);
  ...
  std::shared_ptr<ileqg::PointMassLinearObstacle> pointMassLinearProblem(
    new ileqg::PointMassLinearObstacle(time_horizon, A_rob, B_rob, A_d, A_d,
                        x_d_center, x_obstacle_center, Q_track, Q_track_f, R, Sigma, W, 1e6));
