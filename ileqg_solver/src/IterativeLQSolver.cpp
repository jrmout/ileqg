...
double IterativeLQSolver::solve(LQSolution & sol) {
    ...
    for (int iter = 0 ; iter < solver_params.maxIter ; iter ++) {
        ...
        while (true) {
            ...
            if (cost_new < cost && std::isfinite(cost_new)) {
...
void IterativeLQSolver::simulate(LQSolution & x_u_traj) {
    ...
    for (size_t i = 0; i < T-1 ; i ++) {
        ...
        //std::cout << "Simulation at " << i << ":" << std::endl;
        //std::cout << "X_dot at " << i << " is " << std::endl << x_dot << std::endl << " for x " << std::endl << x_u_traj.x[i] << std::endl << "  and for u " << std::endl << x_u_traj.ff[i] << std::endl;
    }
}
...
double IterativeLQSolver::overallCost(LQSolution & x_u_traj) {
    ...
    for (size_t i = 0; i < T-1 ; i ++) {
        ...
        //std::cout << "Cost to go at " << i << " is "<< cost <<" with state " << x_u_traj.x[i] << " and control " << x_u_traj.ff[i] << std::endl;
    }
    // Final cost
    cost = cost + prob->finalCost(x_u_traj.x[T-1],x_u_traj.ff[T-1]);
    //std::cout << "Final cost is " << cost << " with state " << x_u_traj.x[T-1] << " and control " << x_u_traj.ff[T-1] << std::endl;
