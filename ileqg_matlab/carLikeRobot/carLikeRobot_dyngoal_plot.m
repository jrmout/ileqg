function [ ] = carLikeRobot_dyngoal_plot( x, ~ )
...
for i = 1:T
    ...
    if mod(i,plotStep) == 0
        ...
        if (pseudoGrad == 1)
            ...
            quiver(x_goal(1,i), x_goal(2,i), arrowLength*cos(x_goal(3,i)), arrowLength*sin(x_goal(3,i)), '-','LineWidth',4, 'Color', red_averse, 'AutoScale', 'off', 'MaxHeadSize', 0.9,'LineWidth',3);
            plot(x_goal(1,i), x_goal(2,i), 'o', 'Markersize', 6, 'LineWidth', 5, 'color', red_averse);
        else
            ...
            plot(x_goal(1,i), x_goal(2,i), 'o', 'Markersize', 6, 'LineWidth', 5, 'color', pseudoGrad*red_averse + (1 - pseudoGrad)*(white));
        end
    end
