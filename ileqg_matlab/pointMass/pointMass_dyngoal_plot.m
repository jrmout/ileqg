...
for i = 1:T
    ...
    if mod(i,plotStep) == 0
        ...
        hold on;
        if (i == plotStep)
            plot(x_goal(1,i), x_goal(2,i), 'x', 'Markersize', 6, 'LineWidth', 5, 'color', black, 'Parent', axes_handle);
            plot(x_rob(1,i), x_rob(2,i), 'o', 'Markersize', 6, 'LineWidth', 5, 'color', black, 'Parent', axes_handle);
        else
            if (pseudoGrad < 1)
            ...
            goal = plot(x_goal(1,i), x_goal(2,i), 'x', 'Markersize', 6, 'LineWidth', 5, 'color', pseudoGrad*color_goal + (1 - pseudoGrad)*(light_color_goal), 'Parent', axes_handle);
            set(goal, 'Tag','goal');
            robot = plot(x_rob(1,i), x_rob(2,i), 'o', 'Markersize', 6, 'LineWidth', 5, 'color', pseudoGrad*color_robot + (1 - pseudoGrad)*(light_color_robot), 'Parent', axes_handle);
            set(robot, 'Tag','robot');
        end
    end
