function [] = arm2link_state_dyngoal_plot( x, u)
...
for i = 1:T
    ...
    if mod(i,plotStep) == 0
        ...
        if (pseudoGrad == 1)
            ...
            plot(x(5,i), x(6,i), 'LineWidth', 5, 'color', darkblue);
        else
            plot(taskCoord(1,:), taskCoord(2,:), 'LineWidth', 5, 'color', pseudoGrad*green_seeking + (1 - pseudoGrad)*(white));
            plot(x(5,i), x(6,i), 'o', 'MarkerSize', 6, 'LineWidth', 2, 'color', pseudoGrad*darkred + (1 - pseudoGrad)*(white));
        end
    end
